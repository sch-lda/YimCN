...
namespace big::session
{
	...
	inline void join_session(const rage::rlSessionInfo& info)
	{
		...
		if (SCRIPT::GET_NUMBER_OF_THREADS_RUNNING_THE_SCRIPT_WITH_THIS_HASH(RAGE_JOAAT("maintransition")) == 0)
		{
			...
			g_notification_service->push_error("RID 加入", "战局切换失败");
		}
		return;
	}
	...
	inline void join_by_rockstar_id(uint64_t rid)
	{
		...
		{
			g_notification_service->push_error("RID 加入", "战局过渡正在进行，请稍等");
			return;
		}
		...
		g_notification_service->push_error("RID 加入", "目标玩家不在在线模式");
	}
