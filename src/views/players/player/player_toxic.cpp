...
namespace big
{
	void view::player_toxic()
	{
		if (ImGui::TreeNode("TOXIC"_T.data()))
		{
			...
			ImGui::SameLine();
			components::button("触发制造厂突袭", [] { toxic::start_activity(g_player_service->get_selected(), eActivityType::BikerDefend); });
			ImGui::SameLine();
			components::button("触发地堡突袭", [] { toxic::start_activity(g_player_service->get_selected(), eActivityType::GunrunningDefend); });
			...
			ImGui::SameLine();
			components::player_command_button<"interiortp">(g_player_service->get_selected(), { 81 }, "送进机动作战中心");

			components::player_command_button<"interiortp">(g_player_service->get_selected(), { 123 }, "送进赌场");
			ImGui::SameLine();
			components::player_command_button<"interiortp">(g_player_service->get_selected(), { 124 }, "送进空中别墅");
			ImGui::SameLine();
			components::player_command_button<"interiortp">(g_player_service->get_selected(), { 128 }, "送进游戏厅地下室");

			components::player_command_button<"interiortp">(g_player_service->get_selected(), { 146 }, "送进音乐柜");
			ImGui::SameLine();
			components::player_command_button<"interiortp">(g_player_service->get_selected(), { 148 }, "送进录音A工作室");
			ImGui::SameLine();
			components::player_command_button<"interiortp">(g_player_service->get_selected(), { 149 }, "送进改装铺");

			components::player_command_button<"interiortp">(g_player_service->get_selected(), { 155 }, "送进事务所");
			ImGui::SameLine();
			components::player_command_button<"interiortp">(g_player_service->get_selected(), { 160 }, "送进怪胎店");
			ImGui::SameLine();
			components::player_command_button<"interiortp">(g_player_service->get_selected(), { 161 }, "送进多层车库");
			ImGui::SameLine();
