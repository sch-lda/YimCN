...
namespace big
{
	void view::session()
	{
		...
		ImGui::SameLine();
		components::button("触发制造厂突袭", [] { g_player_service->iterate([](auto& plyr) { toxic::start_activity(plyr.second, eActivityType::BikerDefend); }); });
		ImGui::SameLine();
		components::button("触发地堡突袭", [] { g_player_service->iterate([](auto& plyr) { toxic::start_activity(plyr.second, eActivityType::GunrunningDefend); }); });

		components::command_button<"sextall">({ }, "发送随机短信");
		ImGui::SameLine();
		components::command_button<"fakebanall">({ }, "发送虚假的封号警告");
