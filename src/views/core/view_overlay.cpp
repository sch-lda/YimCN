	...
	void view::overlay()
	{
		...
		if (ImGui::Begin("overlay", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav))
		{
			...
			if (CNetworkPlayerMgr *network_player_mgr = gta_util::get_network_player_mgr(); g.window.ingame_overlay.show_players)
				ImGui::Text(std::format("玩家数量: {}/{}", network_player_mgr->m_player_count, network_player_mgr->m_player_limit).c_str());
			...
			if (auto replay_interface = *g_pointers->m_replay_interface; g.window.ingame_overlay.show_replay_interface)
			{
				...
				if(replay_interface->m_ped_interface)
					ImGui::Text(std::format("生物实体池: {}/{}", replay_interface->m_ped_interface->m_cur_peds, replay_interface->m_ped_interface->m_max_peds).c_str());

				if(replay_interface->m_vehicle_interface)
					ImGui::Text(std::format("载具池: {}/{}", replay_interface->m_vehicle_interface->m_cur_vehicles, replay_interface->m_vehicle_interface->m_max_vehicles).c_str());
					
				if(replay_interface->m_object_interface)
					ImGui::Text(std::format("物体池: {}/{}", replay_interface->m_object_interface->m_cur_objects, replay_interface->m_object_interface->m_max_objects).c_str());
			}
	
			if (g.window.ingame_overlay.show_game_versions)
			{
				ImGui::Separator();
				ImGui::Text(std::format("游戏版本: {}", g_pointers->m_game_version).c_str());
				ImGui::Text(std::format("在线模式版本: {}", g_pointers->m_online_version).c_str());
			}
		}
		ImGui::End();
	}
