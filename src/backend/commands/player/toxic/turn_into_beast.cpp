...
namespace big
{
	class turn_into_beast : player_command
	{
		...
		virtual void execute(player_ptr player, const std::vector<std::uint64_t>& _args, const std::shared_ptr<command_context> ctx)
		{
			...
			if (!NETWORK::NETWORK_IS_PLAYER_A_PARTICIPANT_ON_SCRIPT(id, "am_hunt_the_beast", -1))
			{
				...
				{
					g_notification_service->push_error("变成野兽", "无法启动狩猎野兽活动, player not a participant of am_launcher");
					return;
				}

				g_notification_service->push("变成野兽", "正在启动狩猎野兽活动,请稍后...");
				...
				for (int i = 0; !NETWORK::NETWORK_IS_PLAYER_A_PARTICIPANT_ON_SCRIPT(id, "am_hunt_the_beast", -1); i++)
				{
					if (i >= 1000)
					{
						g_notification_service->push_error("变成野兽", "无法启动狩猎野兽活动");
						return;
					}
			...
			{
				g_notification_service->push_error("变成野兽", "Failed to take control of am_hunt_the_beast");
				return;
			}
	...
	turn_into_beast g_turn_into_beast("beast", "变成野兽", "将玩家变成野兽", 0, false);
	turn_into_beast_all g_turn_into_beast_all("beastall", "将所有人变成野兽", "将所有人包括自己变成野兽", 0);
}